...
describe('RecruitmentParsingService Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    (createConnection as jest.MockedFunction<typeof createConnection>).mockResolvedValue(
      mockConnection
    );
    ...
    (recruitmentParsingService as any).imageProcessor = mockImageProcessor;
  ...
  describe('parseRecruitmentsFromWaitingList', () => {
    it('should process all rows from waiting list successfully', async () => {
      ...
      mockImageProcessor.fetchImage = jest
        .fn<(url: string, id: string) => Promise<void>>()
        .mockResolvedValue(undefined);
      mockImageProcessor.resizeImage = jest
        .fn<(id: string) => Promise<string>>()
        .mockResolvedValue('base64imagestring');
      mockOpenAIClient.textDetectionFromAI = jest
        .fn<(base64: string) => Promise<any>>()
        .mockResolvedValue({
    ...
    it('should fetch and resize images for each recruitment', async () => {
      ...
      expect(mockImageProcessor.fetchImage).toHaveBeenCalledWith(
        'https://example.com/image1.png',
        '101'
      );
      expect(mockImageProcessor.resizeImage).toHaveBeenCalledWith('101');
    });
    ...
    it('should handle empty waiting list', async () => {
      ...
      expect(mockConnection.query).toHaveBeenCalledWith('SELECT * FROM ocr_waiting_list');
      expect(mockImageProcessor.fetchImage).not.toHaveBeenCalled();
